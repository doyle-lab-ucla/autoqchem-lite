...
class molecule_old(object):
    ...
    def __init__(self,
        ...
        # read the molecule
        self.mol = ob_utils.input_to_OBMol(input, input_type, input_format)

        if canonicalize:
            # get canonical smiles of this molecule
            self.can = ob_utils.OBMol_to_string(self.mol, "can")
            logger.info(f"Initializing molecule with canonical smiles: {self.can}")

            # reload molecule from can
            self.mol = ob_utils.input_to_OBMol(self.can, "string", "can")
        else:
            if input_format == "smi":
                self.can = input
            else:
                logger.error(f"Cannot omit canonicalization if input_format is not 'smi'.")
    ...
    def _adjust_geometries(self, min_fragment_dist) -> None:
        ...
        for conf_id in range(self.mol.NumConformers()):
            ...
            init_mdist = distance.cdist(geom.loc[0], geom.loc[1]).min()
            counter = 0
            mdist = init_mdist
            while mdist < min_fragment_dist:
                geom.loc[1] = geom.loc[1].values + 0.1 * v
                mdist = distance.cdist(geom.loc[0], geom.loc[1]).min()
                counter += 1
